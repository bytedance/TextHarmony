...
class MyVQADataset(VQABaseDataset):
    def __init__(self, 
    annt_file, data_root="", transform=None, 
    total_length=None, phase='train', 
    collate_mode='generate_vqa', add_eos=None):
        super().__init__(data_root, annt_file, transform, total_length, phase, collate_mode, add_eos)

    def __getitem__(self, index):
        # image, question, answer
        ann = self.ann[index]
        image = self.loader(ann['image']).convert('RGB')
        image = self.transform(image) if self.transform is not None else image
        question = ann['question']
        answer = ann['answer']
        question_id = index
    ...
    def load_annotations(self):
        return json.load(open(self.annt_file))
...
class DocVQADataset(VQABaseDataset):
    def __init__(
        self,
        data_root='./assets/TextVQA/train_images',
        annt_root='./assets/TextVQA',
        phase='train',
        ann_name_format='docvqa_{split}_annotations.json',
        question_name_format='docvqa_{split}_questions.json',
        **kwargs,
    ):
        self.question_file = os.path.join(annt_root, question_name_format.format(split=phase))
...
class OCRVQADataset(VQABaseDataset):
    def __init__(
        self,
        data_root='./assets/TextVQA/train_images',
        annt_root='./assets/TextVQA',
        phase='train',
        ann_name_format='ocrvqa_{split}_annotations.json',
        question_name_format='ocrvqa_{split}_questions.json',
        **kwargs,
    ):
        self.question_file = os.path.join(annt_root, question_name_format.format(split=phase))
...
class DocVQADataset(VQABaseDataset):
    def __init__(
        self,
        data_root='/mnt/bn/ocr-tangjingqun-cc/mllm-doc/InternLM-XComposer',
        annt_root='/ATA_Benchmark/data/docvqa/test_ans.json',
        phase='test',
        **kwargs,
    ):
        self.question_file = 1
        annt_file = annt_root
        super().__init__(data_root=data_root, annt_file=annt_file, phase=phase, **kwargs)

    def load_annotations(self):
        all_info = json.load(open(self.annt_file))
