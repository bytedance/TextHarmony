...
class LMMTrainer(transformers.Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.is_deepspeed_enabled:
            ...
            with warnings.catch_warnings(record=True) as caught_warnings:
                try:
                    ...
                    print("Failed to save scheduler state-1")
        ...
        if is_torch_tpu_available():
            ...
            try:
                ...
                print("Failed to save optimizer state-1")
            with warnings.catch_warnings(record=True) as caught_warnings:
                try:
                    xm.save(
                        self.lr_scheduler.state_dict(),
                    ...
                    print("Failed to save optimizer state-2")
                reissue_pt_warnings(caught_warnings)
            ...
            if smp.rdp_rank() == 0 or smp.state.cfg.shard_optimizer_state:
                try:
                    ...
                    print("Failed to save optimizer state-3")
            if self.args.should_save:
                with warnings.catch_warnings(record=True) as caught_warnings:
                    try:
                        ...
                        print("Failed to save scheduler state-2")
                reissue_pt_warnings(caught_warnings)
                if self.do_grad_scaling:
                    try:
                        ...
                        print("Failed to save grad scaler state-1")
            ...
            if self.fsdp and not self.is_fsdp_enabled:
                try:
                    ...
                    print("Failed to save optimizer state-4")
            else:
                try:
                    torch.save(
                    ...
                    print("Failed to save optimizer state-5")
            ...
            with warnings.catch_warnings(record=True) as caught_warnings:
                try:
                    ...
                    print("Failed to save lr scheduler-1")
            reissue_pt_warnings(caught_warnings)
            if self.do_grad_scaling:
                try:
                    torch.save(
                    ...
                    print("Failed to save scaler state-1")
        ...
        if self.args.world_size <= 1:
            try:
                ...
                print("Failed to save rng state-1")
        else:
            try:
                torch.save(
                    rng_states,
                ...
                print("Failed to save rng state-2")
    ...
    def training_step(
        ...
        try:
            ...
            with self.compute_loss_context_manager():
                loss, loss_img = self.compute_loss(model, inputs)

            if self.args.n_gpu > 1:
                loss, loss_img = loss.mean(), loss_img.mean()  # mean() to average on multi-gpu parallel training
            ...
            #         if 'lora' in name and ('39' in name or '0' in name):
            #             # print('---------{}--------{}------{}-----'.format(name, param.requires_grad, param.grad))
            #             print("{} {:.12f}".format(name, param.data.reshape(-1).sum().item()))

            return loss.detach() / self.args.gradient_accumulation_steps, \
                loss_img.detach() / self.args.gradient_accumulation_steps
                
        except Exception as e:
            print(e)
            print('Error training')
            return torch.Tensor([0.]).half().to(model.device), torch.Tensor([0.]).half().to(model.device)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_img: torch.Tensor = outputs.pop("loss_img", torch.zeros(1).to(loss.device))

        return ((loss, loss_img), outputs) if return_outputs else (loss, loss_img)
    ...
    def _inner_training_loop(
        ...
        tr_loss_img = torch.tensor(0.0).to(args.device)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                try:
                    ...
                    with self.accelerator.accumulate(model):
                        tr_loss_step, tr_loss_img_step = self.training_step(model, inputs)
                    ...
                    if (
                        ...
                        tr_loss_img = tr_loss_img
                    else:
                        # HACK here
                        # tr_loss += tr_loss_step
                        tr_loss = tr_loss + tr_loss_step
                        tr_loss_img = tr_loss_img_step
                    ...
                    if (
                        ...
                        self._maybe_log_save_evaluate(
                            tr_loss, model, trial, epoch, ignore_keys_for_eval, tr_loss_img=tr_loss_img
                        )
                    else:
                    ...
                    if vis_img and self.accelerator.is_main_process:
                        ...
                        for i, image in enumerate(outputs["image"]):
                            try:
                                ...
                                print(f"{i}th image save failed")
                                continue
                        vis_img = False
                    ...
                    if idx < num_samples:
                        ...
                        if isinstance(image, Image.Image):
                            try:
                                ...
                                print(f"{idx}-th image save failed")
                                continue
                        else:
                            try:
                ...
                for sample_id, (image, caption, data_id) in enumerate(
                    ...
                    if data_id < num_samples:
                        ...
                        try:
                            ...
                            print(f"Failed to save {data_id}.png")
                        ...
                        try:
                            ...
                            print(f"Failed to save {data_id}_segm.png")
                            continue
                        ...
                        if image_id < num_samples:
                            ...
                            if save_gt_image_online:
                                ...
                                try:
                                    ...
                                    print("Failed to save image_gt_path")
                            _result = {
                                "sample_id": meta[0],
                                "frame_idxs": meta[1],
                                "image_id": image_id,
                    ...
                    if image_id < num_samples:
                        ...
                        if save_gt_image_online:
                            ...
                            try:
                                ...
                                print("failed to save image_gt_path-2")
                        _result = {
                            "sample_id": meta[0],
                            "frame_idxs": meta[1],
                            "image_id": image_id,
    ...
    def _maybe_log_save_evaluate(
        self, tr_loss, model, trial, epoch, ignore_keys_for_eval, tr_loss_img=0
    ):
        if self.control.should_log:
            ...
            tr_loss_img_scalar = self._nested_gather(tr_loss_img).mean().item()
