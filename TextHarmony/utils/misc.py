...
def load_model_weights(model, ckpt_path, image_upscale=1.0, use_lora=False, vit_lora=False):
    ...
    if use_lora and vit_lora:
        print("modify ViT weights of pretrained weight to LoRA")
        from copy import deepcopy
        all_del_keys = []
        all_add_kvs = []
        for k in pretrained_weights.keys():
            if k.startswith("visual_tokenizer.encoder"):
                if "base_model.model" not in k:
                    tmp_weight = deepcopy(pretrained_weights[k])
                    cur_k = "visual_tokenizer.encoder" +\
                        ".base_model.model" + k[len("visual_tokenizer.encoder"):]
                    all_del_keys.append(k)
                    all_add_kvs.append((cur_k, tmp_weight))
        for k in all_del_keys:
            del pretrained_weights[k]
        for (cur_k, tmp_weight) in all_add_kvs:
            pretrained_weights[cur_k] = tmp_weight
    ...
    if not is_vit_l:
        pos_embed_key = \
        "visual_tokenizer.encoder.base_model.model.vision_model.embeddings.position_embedding.weight"
        if not (pos_embed_key in pretrained_weights.keys()):
            pos_embed_key = "visual_tokenizer.encoder.pos_embed"
            if not (pos_embed_key in pretrained_weights.keys()):
                pos_embed_key = "visual_tokenizer.encoder.base_model.model.pos_embed"
    print("pos_embed_key: ", pos_embed_key)
    ...
    if old_size != new_size:
        print("change visual_tokenizer.encoder.vision_model.embeddings.position_embedding.weight\n\
            from {} to {}".format((old_size, old_size), (new_size, new_size)))
    ...
    # perceiver query size
    perceiver_query_key = "visual_tokenizer.perceiver_resampler.queries"
    perceiver_query = pretrained_weights[perceiver_query_key].float()
    old_query_size = perceiver_query.size(1)
    new_query_size = model.visual_tokenizer.perceiver_resampler.queries.size(1)
    if old_query_size != new_query_size:
        print("change visual_tokenizer.perceiver_resampler.queries\n\
            from {} to {}".format(old_query_size, new_query_size))
        perceiver_query = perceiver_query.permute(0, 2, 1) # (1, 768, 64)
        perceiver_query = torch.nn.functional.interpolate(
                perceiver_query,
                size=(new_query_size),
                mode="linear",
                align_corners=False,
            ).permute(0, 2, 1) # (1, 256, 768)
        pretrained_weights[perceiver_query_key] = perceiver_query
